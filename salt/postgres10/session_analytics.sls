...
package_session_analytics:
  cmd.run:
    - name: fpm -s dir -t deb -n "postgres10-session-analytics" -v {{ VERSION }} -C /tmp/session_analytics usr
    - creates: /vagrant/postgres10-session-analytics_{{ VERSION }}_amd64.deb
    - cwd: /vagrant
    - user: vagrant
    - require:
      - gem: fpm
